...
class TriangleWave(Wave):
    ...
    def get_sample(self):
        if self.displacement < math.pi:
            return (self.displacement / math.pi) * 2.0

        else:
            reverse = ((self.displacement - math.pi) / math.pi)
            return linear_map(reverse, 0.0, 1.0, 1.0, 0.0)
...
class SawWave(Wave):
    ...
    def get_sample(self):
        return (self.displacement / math.pi) - 1.0
